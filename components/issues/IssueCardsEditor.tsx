      ...
      {issues.map((issue, i) => (
        <div key={i} className="bg-brand-navy-900/40 border border-brand-navy-700 rounded-xl p-4 space-y-3">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <div>
              <label className="text-slate-200 font-medium text-xs mb-1 block">課題カテゴリ</label>
              {(() => {
                ...
                return (
                  <>
                    ...
                    {CATEGORY_EXAMPLES[issue.category] && (
                      <p className="text-slate-400 text-[11px] mt-1 leading-relaxed">相談例: {CATEGORY_EXAMPLES[issue.category]}</p>
                    )}
                  </>
                )
              })()}
            ...
            <div>
              <label className="text-slate-200 font-medium text-xs mb-1 block">種別</label>
              {(() => {
          ...
          <div>
            <label className="text-slate-200 font-medium text-xs mb-1 block">
              タイトル
              <span className="text-slate-400 ml-1">（おせっ会の4名テーブル内で発表するので、一目で経営課題が分かりやすい形に）</span>
            </label>
          ...
          <div>
            <label className="text-slate-200 font-medium text-xs mb-1 block">
              ...
              <span className="text-slate-400 ml-1">（最大250文字）</span>
            </label>
            <textarea value={issue.detail} onChange={e => update(i, { detail: e.target.value })}
              ...
              className="w-full bg-brand-navy-700 border border-brand-navy-700 rounded-lg px-3 py-2 text-white placeholder-slate-500 text-sm focus:outline-none focus:border-brand-sky resize-none" />
            <p className={`text-[11px] mt-1 text-right ${issue.detail.length > 250 ? 'text-red-400' : 'text-slate-400'}`}>
