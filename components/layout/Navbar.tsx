...
const ROLE_LABELS: Record<string, { label: string; className: string }> = {
  ...
  member: { label: '正会員', className: 'bg-brand-sky text-white' },
...
export function Navbar() {
  ...
  return (
    <nav className="bg-brand-navy border-b border-brand-navy-700 sticky top-0 z-50">
            ...
            <Link
              ...
              className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-colors ${
                activeHref === link.href
                  ...
                  : 'text-slate-200 hover:text-white hover:bg-brand-navy-700'
              }`}
            >
              {link.label}
            </Link>
        ...
        <div className="flex items-center gap-2">
          <Link
            ...
            className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-colors ${
              pathname === '/help'
                ? 'bg-brand-sky text-white'
                : 'bg-brand-navy-700 text-slate-200 hover:bg-brand-navy-800 hover:text-white'
            }`}
          >
            ?
          </Link>
          ...
          {session.user?.image ? (
            ...
            <button
              onClick={() => setMenuOpen(!menuOpen)}
              className="w-8 h-8 bg-brand-sky rounded-full flex items-center justify-center text-white text-sm font-bold"
            >
              {(session.user?.name ?? 'U')[0]}
            </button>
          )}
          {menuOpen && (
            <div className="absolute top-14 right-4 bg-brand-navy-800 border border-brand-navy-700 rounded-xl shadow-xl py-1 w-44 z-50">
              <div className="px-3 py-2 border-b border-brand-navy-700">
                <p className="text-white text-sm font-medium truncate">{session.user?.name}</p>
                <p className="text-slate-400 text-xs truncate">{session.user?.email}</p>
              </div>
              <Link
                href="/mypage"
                className="block px-3 py-2 text-sm text-slate-200 hover:text-white hover:bg-brand-navy-700"
              ...
              <div className="md:hidden border-t border-brand-navy-700">
                {links.map(link => (
                  <Link
                    key={link.href}
                    href={link.href}
                    className="block px-3 py-2 text-sm text-slate-300 hover:text-white hover:bg-brand-navy-700"
                    onClick={() => setMenuOpen(false)}
                  >
                    {link.label}
                  </Link>
              ...
              <button
                ...
                className="w-full text-left px-3 py-2 text-sm text-red-400 hover:text-red-300 hover:bg-brand-navy-700"
              >
                ログアウト
              </button>
            </div>
