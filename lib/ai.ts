...
const PROFILE_LABELS: Record<string, string> = {
  ...
  targetRevenueScale: 'クライアントの平均売上規模',
...
- detail: 背景の説明（現状の具体的な数字・取り組み・これまで試したことをプロフィール情報も活かして記述）と、「具体的に何を聞きたいか／どんな相手を紹介してほしいか」を明確に書く。1〜3文。**必ず250文字以内に収める。**
...
export async function analyzeToIssues(p: AnalyzeParams): Promise<AiIssue[]> {
  ...
  return issues.map(i => ({
    ...
    detail: String(i.detail ?? '').slice(0, 250),
  }))
}
